			...
			@if(Auth::user()->role!='employee')
				@if($Adjustmentamount != 0)
					...
					<form method="post" action="{{ url('invoice/stripe')}}" class="medium" id="medium">
					  ...
					  <input type="submit" class="submit2  btn btn-default text-right" value="{{ trans('app.Pay with card')}}" data-key="<?php echo $p_key; ?>" data-email="{{$customer->email}}" 
						 data-name="{{$logo->system_name}}"data-description="Invoice Number -{{$service_tax->invoice_number}}" data-amount="{{$Adjustmentamount * 100}}"  />
